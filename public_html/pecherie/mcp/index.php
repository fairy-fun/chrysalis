...
// ─── Bearer token validation — shared secret from config ─────────────────────
...
function validateBearerToken(): bool
{
    ...
    if ($expected === '') {
        error_log('MCP auth: mcp_api_key is empty in config');
        return false;
    }
    ...
    error_log('MCP auth: provided token length=' . ($provided !== null ? strlen($provided) : 'null') . ' expected length=' . strlen($expected));
